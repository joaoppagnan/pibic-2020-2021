...
class ModeloESN():
    ...
    def predicao(self, X_teste, n_transitory_samples):
        ...
        for l in range(0,self.Nl):
            ...
            # elimina a primeira linha (estado inicial com zeros) 
            xt = np.delete(x_l, 0, 0)
        ...
        for l in range(0, self.Nl):
            ...
            Xaux = self.X_teste[l]
            xt = np.delete(Xaux, np.arange(n_transitory_samples), 0)
            self.Xvec_teste[:, :l*self._n_neurons:(l+1)*self._n_neurons] = xt.flatten
