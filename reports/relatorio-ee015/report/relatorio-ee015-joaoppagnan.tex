...
O formato do atrator varia bastante de acordo com o sistema em análise. Por exemplo, um circuito RC (resistor conectado em série com um capacitor) tem um atrator chamado de ponto-fixo. Isso é devido ao fato de que, dada uma tensão inicial no capacitor (ou seja, uma energia inicial presente no sistema), o resistor dissipará a energia armazenada em forma de potência ativa (calor). Considerando as variáveis do espaço de estado como sendo a tensão $V$, e a corrente $I$ do circuito, é fácil perceber que a trajetória vai convergir para o ponto $(0, 0)$, pois a energia será toda dissipada e não haverá mais dinâmica nesse sistema.

A Figura mostra a evolução de um circuito RC em série para $V_{C}(0) = \SI{1}{\volt}$, com $R = \SI{1}{\ohm}$ e $C = \SI{1}{F}$:

Diferentemente dos sistemas dinâmicos lineares que, através de uma análise dos autovalores da matriz que descreve as equações do sistema, pode-se concluir que eles apresentam algum tipo de atrator de trajetórias facilmente determinável, sendo este uma divergência de trajetórias para o infinito, um ponto fixo ao qual todas as trajetórias convergem, um ponto de sela que uma trajetória movendo-se em uma direção e uma outra tra, entre outros, os sistemas caóticos apresentam um comportamento errático, sem pontos fixos, ciclos-limite, pontos de sela, ou outras estruturas topológicas comumente estudadas em cursos de análise de sistemas \cite{fiedler1994caos}.
